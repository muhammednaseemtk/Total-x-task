...
class WelcomeTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const Column(
      children: [
        Text('Welcome to', style: TextStyle(fontSize: 18, color: AppColors.textSecondary)),
        SizedBox(height: 8),
        Text('TOTAL-X', style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: AppColors.primary, letterSpacing: 2)),
        SizedBox(height: 16),
        Text('Sign in to manage users\nand access your dashboard', textAlign: TextAlign.center, style: TextStyle(fontSize: 16, color: AppColors.textSecondary, height: 1.5)),
      ],
    );
  }
}
...
class TermsText extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return const Text('By continuing, you agree to our\nTerms of Service and Privacy Policy', textAlign: TextAlign.center, style: TextStyle(fontSize: 12, color: AppColors.grey, height: 1.5));
  }
}
...
class AccountTileItem extends StatelessWidget {
  ...
  const AccountTileItem({super.key, required this.account, required this.onTap});
    ...
    return Container(
      ...
      child: ListTile(
        leading: CircleAvatar(
          ...
          child: account.photoUrl == null ? Text(account.displayName?.substring(0, 1).toUpperCase() ?? '?', style: const TextStyle(color: AppColors.white)) : null,
        ),
...
class AccountList extends StatelessWidget {
  ...
  const AccountList({super.key, required this.accounts, required this.onSelect, required this.onAddAnother});

  @override
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...accounts.map((account) => AccountTileItem(account: account, onTap: () => onSelect(account))),
        const SizedBox(height: 16),
        TextButton(onPressed: onAddAnother, child: const Text('Sign in with a different account')),
      ],
    );
  }
}
...
class SearchFilterBar extends StatelessWidget {
  ...
  const SearchFilterBar({super.key, required this.controller, required this.onSearch, required this.onClear, required this.onFilter});
            ...
            child: Container(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: controller.text.isNotEmpty ? IconButton(icon: const Icon(Icons.clear, color: AppColors.grey), onPressed: onClear) : null,
...
class UserAvatar extends StatelessWidget {
  ...
  Widget initialsWidget() {
    return Center(
      child: Text(Helpers.getInitials(user.name), style: TextStyle(fontSize: size * 0.35, fontWeight: FontWeight.bold, color: AppColors.white)),
    );
  }
}
...
class AgeTagBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: user.isOlder ? Colors.orange.withAlpha(26) : Colors.green.withAlpha(26),
        borderRadius: BorderRadius.circular(4),
      ),
      child: Text(
        '${user.age} yrs (${Helpers.formatAgeCategory(user.age)})',
        style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: user.isOlder ? Colors.orange[700] : Colors.green[700]),
      ),
    );
  }
}
...
class EmptyStateView extends StatelessWidget {
  ...
  const EmptyStateView({super.key, required this.title, required this.subtitle, this.icon = Icons.people_outline});
...
class BottomSheetHandle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Container(width: 40, height: 4, decoration: BoxDecoration(color: AppColors.grey.withAlpha(77), borderRadius: BorderRadius.circular(2))),
    );
  }
}
...
class SortOptionItem extends StatelessWidget {
  ...
  const SortOptionItem({super.key, required this.title, required this.isSelected, required this.onTap});
...
class FormTextField extends StatelessWidget {
  ...
  const FormTextField({super.key, required this.controller, required this.label, required this.icon, this.keyboardType = TextInputType.text, this.validator});
...
class AppButton extends StatelessWidget {
  ...
  const AppButton({super.key, required this.text, required this.onPressed, this.isLoading = false, this.isOutlined = false});

  @override
  Widget build(BuildContext context) {
    return isOutlined
        ? TextButton(onPressed: onPressed, child: Text(text))
        : ElevatedButton(onPressed: isLoading ? null : onPressed, child: isLoading ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2)) : Text(text));
  }
}